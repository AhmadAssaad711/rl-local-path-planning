...
def train_and_evaluate(args: argparse.Namespace) -> dict:
    ...
    # Default to 24 parallel subprocess environments here so DQN rollout
    # collection is explicit and matches the Kourani-style CPU parallelism.
    if args.num_envs == 1:
        env = make_vec_env(
            "highway-v0",
            n_envs=1,
        ...
        print(f"Spawning {args.num_envs} parallel DQN environments")
        env = make_vec_env(
            "highway-v0",
